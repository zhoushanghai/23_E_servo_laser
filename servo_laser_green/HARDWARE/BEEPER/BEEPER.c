...
void TIM2_PWM_Init(u16 arr)
{
    ...
    TIM_OCInitStructure.TIM_Pulse = arr - 100; // 占空比
    TIM_OC2Init(TIM2, &TIM_OCInitStructure);

    TIM_CtrlPWMOutputs(TIM2, ENABLE);

    TIM_Cmd(TIM2, DISABLE); // 先关掉防止蜂鸣器怪叫
}
...
// 调用此函数即可发出声音，前提是将Beeper_Proc函数直接放入1ms定时器中断内
void Beeper_Red_Green_Traced(void)
{
    Beeper_Perform(BEEPER_RED_GREEN_TRACE);
}
...
void Beeper_Proc(void)
{
    ...
    if (Beeper0.Beeper_Continue_Flag && Beeper0.Beeper_Enable)
    {
        ...
        {
            /*给预重装载值赋值，改变音调*/
            TIM2->ARR = (uint16_t)Set_Musical_Note(MusicNoteFrequency[MySound[Beeper0.Beep_Play_Schedule].Note]);
            /*给PWM占空比赋值，改变音量*/
            TIM2->CCR2 = (uint16_t)TIM2->ARR - 100;
            /*时间减短1ms*/
